...
corpus_stopwords = ["category", "references", "also", "external", "links", "may", "first", "follower", "see", "history",
...
def get_topN_score_for_queries_big(query, index, dir1, exist_doc = {}):
    """
    Generate a dictionary that gathers for every query its topN score.
    ...
    for qi in query:
        if qi not in all_stopwords:
            ...
            if df != 0:
                list_of_doc = get_posting_iter(index, qi, dir1)
                for doc, freq in list_of_doc:
                    if doc not in exist_doc:
                        sim[doc] = sim.get(doc, (pageRank_dict.get(str(doc),0) / max_rank + pageView_dict.get(doc,0) / 181126232))
                    sim[doc] = sim.get(doc, 0) + 1
    return sim

# merge
def try_merge_results(title_scores, dic_pls, body_scores_cosine, anchor_scores, num, query_tok, title_weight=0.23,
                      body_weight_BM25=0.37, body_weight_cosine=0.13, anchor_weight=0.1):
    """
    ...
    for doc in query_:
        ...
        score = body_scores_BM25.get(doc, 0) * body_weight_BM25 #+ (pageView_dict[doc] * 0.19 / 181126232)
...
def search():
    ...
    tokens = [stemmer.stem(token) for token in query1]
    new = query1 + tokens
    query_tok = set(new)
    count = 0
    ...
    title_dic = get_topN_score_for_queries_big(query_tok, idx_title, './indexTitle/postings_gcp')
    _temp_t = sorted([(doc_id, score) for doc_id, score in title_dic.items()], key=lambda x: x[1], reverse=True)[:100]
    cosine_dic = get_topN_score_for_queries_body_cosine(query_tok, idx_body, './indexBody/postings_gcp', dic_pls)
    anchor_dic = get_topN_score_for_queries_big(query_tok, idx_anchor, './indexAnchor/postings_gcp', dict(_temp_t))
    _temp_a = sorted([(doc_id, score) for doc_id, score in anchor_dic.items()], key=lambda x: x[1], reverse=True)[:800]

    merge = try_merge_results(_temp_t, dic_pls, cosine_dic, _temp_a, count, query_tok)
    temp = sorted([(doc_id, score) for doc_id, score in merge.items()], key=lambda x: x[1], reverse=True)[:50]
